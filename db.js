...
export const getAuth = async (apiKey) => {
	const { rows } = await db.query("select * from tikshared.api_keys where api_key = $1", [apiKey]);
	const db_key = rows.length ? rows[0] : null;
...
export const getIfSaved = async (url) => {
	...
	const { rows } = await db.query(`select name from tikshared.videos where url = $1`, [url]);
	if (rows.length) {
		return rows[0].name;
	}
	return false;
...
export const getVideoCount = async (name) => {
	const { rows } = await db.query("select count(*) as count from tikshared.views where name = $1", [name]);
	if (rows.length) {
		return rows[0].count;
	}
	return 0;
